    ...
    getgenv().originalNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if method == "FireServer" then
            for _, rule in ipairs(getgenv().HookRules) do
                -- Nếu rule có remoteName thì so khớp theo tên Remote
                if (not rule.remoteName or self.Name == rule.remoteName) then
                    -- Nếu rule có blockedFirstArg thì so khớp thêm args[1]
...
-- Chặn Guest1337Collision (Charge)
...
-- Protection (DusekkarCancel auto fire lại sau 4s)
